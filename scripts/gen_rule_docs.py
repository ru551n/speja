...
``speja --list_rules``, and CI fails when the committed output no longer matches.
...
def list_rules() -> list[tuple[str, str, str]]:
    ...
    binary = ROOT / "target" / "release" / "speja"
...
def cli_reference() -> str:
    ...
    vsg = [(f, d) for f, d in options if "(speja extension)" not in d]
    ours = [(f, d.replace(" (speja extension)", "")) for f, d in options if "(speja extension)" in d]
...
def counts_page(rules: list[tuple[str, str, str]]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"| — of those, native to speja | {n['native']} |",
            "",
        ]
    )
