...
`speja` formats VHDL and reports what is wrong with it. One binary, two layers: **style** (the
...
```sh
speja --version    # not installed: `pip install speja` or `uv tool install speja`
```
...
speja --fix $files                    # format, and apply every safe fix
speja --check style,lint $files       # what is left, from both layers
```
...
So anything a default run reports is a definite error. `speja --explain <rule>` gives a rule's
...
|---|---|
| `speja --fix FILE...` | format and apply every safe fix, in place |
| `speja --fix --diff FILE...` | the same as a unified diff, changing nothing |
| `speja --check style,lint FILE...` | report both layers |
| `speja lint FILE...` | the lint layer alone |
| `speja --recursive src` | every `.vhd` and `.vhdl` under a directory |
| `speja --explain RULE` | what one rule means |
| `speja --list_rules` | every rule, its layer, its class, and whether it is on |
...
This plugin registers the `speja` MCP server, which answers the same things through tools. One
