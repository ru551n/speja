...
| `speja -f` over VUnit's VHDL (222 files, 3.4 MB) | 0.59 s |
| `speja lint` over VUnit's VHDL (493 files) | 0.59 s |
...
per core than running them one at a time. `speja` starts worker processes instead, each
