...
speja and [vhdl_ls](https://github.com/VHDL-LS/rust_hdl) share a front end and divide the work.
...
| **speja** | how the source should look and what it does wrong: formatting, style rules, static analysis, fixes |

`speja lsp` advertises only the second half, and [says so in its capabilities](lsp.md): it
...
[`speja mcp`](mcp.md) is the same half again, for a reader that is a coding agent rather than an
...
column does not move into speja because the crate that could implement it is linked already. If
speja ever answers a navigation request, that is the boundary breaking, whatever the dependency
graph says.

The cost of sharing is real and accepted: `vhdl_syntax` decides which VHDL speja can parse (see
...
A single package (`speja`) with a library (`speja`) and a binary (`speja`, VSG's command
