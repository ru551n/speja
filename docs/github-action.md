...
`ru551n/speja` is also a GitHub Action. It downloads the speja release binary (checked
...
|---|---|---|
| `args` | `--recursive .` | speja arguments (VSG's command line); `--sarif` is added |
| `version` | the action's tag, else `latest` | speja release to download |
| `working-directory` | `.` | where speja runs, relative to the repository root |
...
| `fail-on-violations` | `true` | fail the job when speja exits with 1 |
...
that the pull request shows and that still read as speja saw them; at most 50 per run, and a
suggestion that was already posted is not repeated. On every run, the action resolves its
earlier suggestion threads that speja no longer makes (the problem was fixed, or the lines
...
With `sarif-upload: true` (or `speja --sarif FILE` followed by
`github/codeql-action/upload-sarif`), each violation becomes a code scanning alert:

* speja reports one result per rule violation, and one `format` result per block of adjacent
...
the category `speja`, so they do not replace the results of other tools such as CodeQL.
