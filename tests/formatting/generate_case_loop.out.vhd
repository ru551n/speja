...
begin
  ...
  gen_lanes : for lane_index in 0
      to number_of_lanes_in_the_design - 1 generate
    signal lane_data : std_logic_vector(7 downto 0);
  begin
    lane_data <= input_data(lane_index);
  end generate gen_lanes;

  gen_opt : if enable_optional_feature_one
               and enable_optional_feature_two generate
    feature <= '1';
  ...
  begin
    ...
    for element_index in input_array'low
                         to input_array'high
                            - number_of_skipped_elements loop
